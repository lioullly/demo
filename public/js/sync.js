...
import { WS_PORT, setConnectedHost, connectedHost, cv, strokes, USER_ID } from './utils.js'
...
export function connectWS(host, room) {
  ...
  return new Promise((resolve) => {
    ...
    const fail = () => {
      if (cid !== _connectId) return // Only handle the latest connection attempt
      if (settled) {
        // Connection was established, but later dropped — update UI
        const sd = document.getElementById('status-dot')
      ...
      // Never connected — clean up
      settled = true; window.ws = null; resolve(false)
      const sd = document.getElementById('status-dot')
      if (sd) { sd.textContent = 'Disconnected'; sd.className = 'status-wait' }
      const rb = document.getElementById('btn-reconnect'); if (rb) rb.style.display = ''
    }
    window.ws.onerror = fail
    window.ws.onclose = () => { fail() }
    window.ws.onmessage = handleWS
    setTimeout(() => { if (!settled && cid === _connectId) { settled = true; window.ws = null; resolve(false) } }, 5000)
  })
}
...
function redrawBoard(board) {
  ...
  strokes[board].forEach((s) => draw(ctx, s.points, s.color, s.width || s.size))
  ctx.restore()
}
